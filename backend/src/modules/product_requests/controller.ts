...
import { productRequestItems, productRequests, products, users } from "backend/drizzle/schema";
...
export const productRequestsController = new Elysia({
    ...
    .get("/product_requests", async ({
        ...
        if (dateFrom && dateTo) {
            whereClause.push(...[
                gte(productRequests.created_at, new Date(dateFrom)),
                lte(productRequests.created_at, new Date(dateTo))
            ]);
        }
        ...
        const [requests, total] = await Promise.all([
            ...
            drizzle.select({ count: sql<number>`count(*)` })
                .from(productRequests)
                .where(and(...whereClause))
                .execute()
        ]);
    ...
    .get("/product_requests/:id", async ({
      ...
      `,
            ...
            .from(productRequests)
            .leftJoin(productRequestItems, eq(productRequests.id, productRequestItems.requestId))
            .leftJoin(products, eq(productRequestItems.productId, products.id))
            .where(eq(productRequests.id, id))
            .groupBy(productRequests.id, productRequests.status, productRequests.created_at, productRequests.firstName, productRequests.lastName)
            .execute();
