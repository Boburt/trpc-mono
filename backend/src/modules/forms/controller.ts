...
import { sql, InferSelectModel, InferInsertModel, eq } from "drizzle-orm";
...
export const formsController = new Elysia({
    ...
    async ({
        ...
        console.log('getting form', drizzle
            .select()
            .from(forms)
            .where(eq(forms.id, id)).toSQL().sql)
        console.log('getting form', drizzle
            .select()
            .from(forms)
            .where(eq(forms.id, id)).toSQL().params)
    ...
    async ({
        ...
        if (!user.permissions.includes("forms.update")) {
            set.status = 401;
            return {
                message: "You don't have permissions",
            };
