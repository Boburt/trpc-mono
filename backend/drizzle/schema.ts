import { pgTable, foreignKey, pgEnum, uuid, text, integer, timestamp, uniqueIndex, index, boolean, jsonb, doublePrecision, varchar, numeric, unique, primaryKey } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const manufacturers_properties_types = pgEnum("manufacturers_properties_types", ['string', 'number', 'boolean', 'date', 'list'])
...
export const categories = pgTable("categories", {
  ...
  (table) => {
    return {
      ...
      idx_categories_parent_id: index("idx_categories_parent_id").using("btree", table.parent_id),
    }
  });
...
export const forms = pgTable("forms", {
  ...
  updated_by: uuid("updated_by").references(() => users.id, { onUpdate: "cascade" }),
  updated_at: timestamp("updated_at", { precision: 5, withTimezone: true, mode: 'string' }),
  schedule_time: timestamp("schedule_time", { precision: 5, withTimezone: true, mode: 'string' }),
});
...
export const forms_sent_items = pgTable("forms_sent_items", {
  ...
  status: text("status").notNull(),
  opened_by: uuid("opened_by").references(() => users.id, { onUpdate: "cascade" }),
  applied_by: uuid("applied_by").references(() => users.id, { onUpdate: "cascade" }),
  created_at: timestamp("created_at", { precision: 5, withTimezone: true, mode: 'string' }),
  updated_at: timestamp("updated_at", { precision: 5, withTimezone: true, mode: 'string' }),
  opened_at: timestamp("opened_at", { precision: 5, withTimezone: true, mode: 'string' }),
});
...
export const participants = pgTable("participants", {
  ...
  joined_at: timestamp("joined_at", { precision: 5, withTimezone: true, mode: 'string' }),
  left_at: timestamp("left_at", { precision: 5, withTimezone: true, mode: 'string' }),
});
...
export const products_categories = pgTable("products_categories", {
  id: uuid("id").defaultRandom().primaryKey().notNull(),
  product_id: uuid("product_id").notNull().references(() => products.id, { onUpdate: "cascade" }),
  category_id: uuid("category_id").notNull().references(() => categories.id, { onUpdate: "cascade" }),
  created_at: timestamp("created_at", { precision: 5, withTimezone: true, mode: 'string' }).defaultNow().notNull(),
  updated_at: timestamp("updated_at", { precision: 5, withTimezone: true, mode: 'string' }).defaultNow().notNull(),
},
  (table) => {
    return {
      idx_products_categories_category_id: index("idx_products_categories_category_id").using("btree", table.category_id),
    }
  });
...
export const products_properties = pgTable("products_properties", {
  id: uuid("id").defaultRandom().primaryKey().notNull(),
  product_id: uuid("product_id").notNull().references(() => products.id, { onUpdate: "cascade" }),
  property_id: uuid("property_id").notNull().references(() => properties.id, { onUpdate: "cascade" }),
  value: text("value").notNull(),
  created_at: timestamp("created_at", { precision: 5, withTimezone: true, mode: 'string' }).defaultNow().notNull(),
  updated_at: timestamp("updated_at", { precision: 5, withTimezone: true, mode: 'string' }).defaultNow().notNull(),
});
...
export const products = pgTable("products", {
  ...
  price_rub: numeric("price_rub"),
  price_usd: numeric("price_usd"),
},
  (table) => {
    return {
      idx_products_active: index("idx_products_active").using("btree", table.active),
    }
  });
...
export const users = pgTable("users", {
  ...
  (table) => {
    return {
      UQ_0e2c0e1b3b0b0b0b0b0b0b0b0b0: uniqueIndex("UQ_0e2c0e1b3b0b0b0b0b0b0b0b0b0").using("btree", table.login),
...
export const memberships = pgTable("memberships", {
  ...
  (table) => {
    return {
      ...
      memberships_short_name_unique: unique("memberships_short_name_unique").on(table.short_name),
    }
  });
...
export const profiles = pgTable("profiles", {
  ...
  references_id: uuid("references_id").notNull().references(() => memberships.id, { onUpdate: "cascade" }),
  created_at: timestamp("created_at", { precision: 5, withTimezone: true, mode: 'string' }).defaultNow().notNull(),
  updated_at: timestamp("updated_at", { precision: 5, withTimezone: true, mode: 'string' }).defaultNow().notNull(),
});
...
export const locations = pgTable("locations", {
  ...
  location_types: varchar("location_types", { length: 255 }).notNull(),
  ...
  (table) => {
    return {
      country_id_idx: index("country_id_idx").using("btree", table.country_id),
      state_id_idx: index("state_id_idx").using("btree", table.state_id),
      type_idx: index("type_idx").using("btree", table.location_types),
    }
  });

export const membership_users = pgTable("membership_users", {
  id: uuid("id").defaultRandom().primaryKey().notNull(),
  membership_id: uuid("membership_id").notNull().references(() => memberships.id, { onUpdate: "cascade" }),
  user_id: uuid("user_id").notNull().references(() => users.id, { onUpdate: "cascade" }),
  post: text("post"),
  is_admin: boolean("is_admin").default(false).notNull(),
  created_at: timestamp("created_at", { precision: 5, withTimezone: true, mode: 'string' }).defaultNow().notNull(),
...
export const roles_permissions = pgTable("roles_permissions", {
  ...
  (table) => {
    return {
      roles_permissions_role_id_permission_id_pk: primaryKey({ columns: [table.role_id, table.permission_id], name: "roles_permissions_role_id_permission_id_pk" }),
    }
  });
...
export const users_permissions = pgTable("users_permissions", {
  ...
  (table) => {
    return {
      users_permissions_user_id_permission_id_pk: primaryKey({ columns: [table.user_id, table.permission_id], name: "users_permissions_user_id_permission_id_pk" }),
    }
  });
...
export const users_roles = pgTable("users_roles", {
  ...
  (table) => {
    return {
      users_roles_user_id_role_id_pk: primaryKey({ columns: [table.user_id, table.role_id], name: "users_roles_user_id_role_id_pk" }),
    }
  });
...
export const productRequestItems = pgTable("product_request_items", {
  ...
  (table) => {
    return {
      product_requests_items_id_created_at_pk: primaryKey({ columns: [table.id, table.created_at], name: "product_requests_items_id_created_at_pk" }),
    }
  });
...
export const messages = pgTable("messages", {
  ...
  is_deleted: boolean("is_deleted").default(false).notNull(),
  sent_at: timestamp("sent_at", { precision: 5, withTimezone: true, mode: 'string' }).defaultNow().notNull(),
  edited_at: timestamp("edited_at", { precision: 5, withTimezone: true, mode: 'string' }),
},
  (table) => {
    return {
      sent_at_idx: index("messages_sent_at_idx").using("btree", table.sent_at),
      messages_id_sent_at_pk: primaryKey({ columns: [table.id, table.sent_at], name: "messages_id_sent_at_pk" }),
    }
  });
...
export const productRequests = pgTable("product_requests", {
  ...
  created_at: timestamp("created_at", { mode: 'string' }).defaultNow().notNull(),
  request_number: integer("request_number").default(
    sql`nextval('request_number_seq')`
  ),
},
  (table) => {
    return {
      product_requests_id_created_at_pk: primaryKey({ columns: [table.id, table.created_at], name: "product_requests_id_created_at_pk" }),
    }
  });
...
export const product_events = pgTable("product_events", {
  ...
  (table) => {
    return {
      product_events_id_created_at_pk: primaryKey({ columns: [table.id, table.created_at], name: "product_events_id_created_at_pk" }),
    }
  });