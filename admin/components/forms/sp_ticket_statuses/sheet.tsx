...
import SpTicketStatusesForm from "./_form";

export default function SpTicketStatusesFormSheet({
  children,
  recordId,
}: {
  ...
  return (
    <Sheet onOpenChange={beforeOpen} open={open}>
      ...
      <SheetContent>
        <SheetHeader>
          <SheetTitle>{recordId ? "Edit" : "Add"} Ticket Status</SheetTitle>
        </SheetHeader>
        {open && <SpTicketStatusesForm setOpen={setOpen} recordId={recordId} />}
      </SheetContent>
    </Sheet>
  );
}
