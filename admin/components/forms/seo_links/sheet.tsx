...
import PermissionsForm from "./_form";

export default function SeoLinksFormSheet({
  children,
  recordId,
}: {
  ...
  return (
    <Sheet onOpenChange={beforeOpen} open={open}>
      ...
      <SheetContent>
        <SheetHeader>
          <SheetTitle>{recordId ? "Edit" : "Add"} Seo Links</SheetTitle>
        </SheetHeader>
        {open && <PermissionsForm setOpen={setOpen} recordId={recordId} />}
      </SheetContent>
    </Sheet>
  );
}
