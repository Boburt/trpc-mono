...
import CitiesForm from "./_form";

export default function CitiesFormSheet({
  children,
  recordId,
}: {
  ...
  return (
    <Sheet onOpenChange={beforeOpen} open={open}>
      ...
      <SheetContent>
        <SheetHeader>
          <SheetTitle>{recordId ? "Edit" : "Add"} City</SheetTitle>
        </SheetHeader>
        {open && <CitiesForm setOpen={setOpen} recordId={recordId} />}
      </SheetContent>
    </Sheet>
  );
}
