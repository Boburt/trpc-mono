...
import ImageSizesForm from "./_form";

export default function ImageSizesFormSheet({
  children,
  recordId,
}: {
  ...
  return (
    <Sheet onOpenChange={beforeOpen} open={open}>
      ...
      <SheetContent>
        <SheetHeader>
          <SheetTitle>{recordId ? "Edit" : "Add"} Image Sizes</SheetTitle>
        </SheetHeader>
        {open && <ImageSizesForm setOpen={setOpen} recordId={recordId} />}
      </SheetContent>
    </Sheet>
  );
}
