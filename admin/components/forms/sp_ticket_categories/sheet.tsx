...
import SpTicketCategoriesForm from "./_form";

export default function SpTicketCategoriesFormSheet({
  children,
  recordId,
}: {
  ...
  return (
    <Sheet onOpenChange={beforeOpen} open={open}>
      ...
      <SheetContent>
        <SheetHeader>
          <SheetTitle>{recordId ? "Edit" : "Add"} Ticket Categories</SheetTitle>
        </SheetHeader>
        {open && (
          <SpTicketCategoriesForm setOpen={setOpen} recordId={recordId} />
        )}
      </SheetContent>
    </Sheet>
  );
